...
df_list <- purrr::map(files, ~ readr::read_csv(here::here("..", "raw_data", .x), show_col_types = F) |>
  janitor::clean_names())
...
# AQ 2023
df23 <- df23 |> 
  dplyr::mutate(income = ifelse(is.na(income), income2022, income)) 

# LHES
dflf <- dflf |> 
  dplyr::select(pid, immstatus) |> 
  dplyr::group_by(pid) |>
  dplyr::mutate(pid_count = n()) |> 
  dplyr::filter(!(pid_count > 1 & is.na(immstatus))) |> 
  dplyr::ungroup() |> 
  dplyr::distinct(pid, .keep_all = T) |> 
  dplyr::select(-pid_count)
```

Select all variables, including exposure `occ`, outcome `phq`, and potential confounders: age, region, rural, education,
immigration status, minority stress, and job discrimination. Row bind AQ 2021 - 2023 into a single data frame for data 
cleaning.
```{r}
...
v_names <- df22 |>
  dplyr::select(
    ...
    genderid_1:race_ethn_8_text, saab, selfbin_gi, selfbin_so, ed_level,
...
df_long <- purrr::map2_dfr(dfs, c(2021, 2022, 2023), ~ .x |>
  dplyr::filter(finished == 1) |>
  dplyr::select(dplyr::all_of(v_names)) |>
  dplyr::mutate(
...
df_long <- df_long |> 
  dplyr::mutate(
    # PHQ-9 scores
    phq = rowSums(dplyr::across(phq1:phq9)), 
    # Count how many occupation types in main categories
    occ_main_sum = rowSums(dplyr::across(occ_1:occ_5), na.rm = T),
    occ_total_sum = rowSums(dplyr::across(occ_1:occ_11), na.rm = T),
    # Employment status:
    # 1=standard, 2=parttime, 3=temp, 4=self-employed, 5=unemployed, 6=nonstandard, 7=out of workforce
    occ_cat = dplyr::case_when(
      occ_total_sum == 0 ~ NA,
    ...
    student = dplyr::case_when(occ_8 == 1 | occ_9 == 1 ~ 1, T ~ 0),
    # Binary indicator reporting unemployed or not looking for work
    any_unemployed = dplyr::case_when(occ_5 == 1 | occ_6 == 1 ~ 1, T ~ 0)
  )
```
...
df_long <- df_long |> 
  dplyr::mutate(
    ...
    cars_sm = rowSums(dplyr::across(cars1sm:cars5sm)),
    cars_gm = rowSums(dplyr::across(cars1gm:cars5gm)),
  ...
  dplyr::select(
    ...
    yrjobdisc, cars, phq, occ_cat,
    out_work_force, employed, student, any_unemployed, occ_1:occ_11
  )
```
...
v_names <- df_long |> dplyr::select(-pid, -survey_yr) |> names()

# Identify participants who completed 2 or more annual questionnaires
complete_pids <- df_long |> 
  dplyr::group_by(pid) |> 
  dplyr::filter(dplyr::n_distinct(survey_yr) >= 2) |> 
  dplyr::pull(pid) |> 
  unique()

# Pivot wider
df_wide <- df_long |> 
  tidyr::pivot_wider(names_from = survey_yr, 
              values_from = dplyr::all_of(v_names)) |> 
  ...
  dplyr::mutate(
    ...
    # Exclude flag for sensitivity analysis: People who unemployed AND fulltime/parttime/temp/self employed
    # exclude_flag_sen1 = dplyr::case_when(

    # )    
  ) |> 
  # Cross stratify employment from AQ 2021 and 2022
  dplyr::mutate(emp_chg = dplyr::case_when(
  ...
  dplyr::filter(!is.na(employed_2021), !is.na(employed_2022)) 
...
v_names <- df21 |> 
  dplyr::select(
    ...
    genderid_1:genderid_12, saab, selfbin_gi,
    orientation_1:orientation_11, selfbin_so
  ) |> 
  names()

df_demo_21 <- df21 |> dplyr::select(dplyr::all_of(v_names))
df_demo_22 <- df22 |> dplyr::select(dplyr::all_of(v_names))
df_demo_23 <- df23 |> dplyr::select(dplyr::all_of(v_names))

# Create a named list of data frames
df_demo_list <- list(df_demo_21 = df_demo_21, df_demo_22 = df_demo_22, df_demo_23 = df_demo_23)

# Apply the function and attach the respective year
df_demo <- purrr::map2_dfr(df_demo_list, c(2021, 2022, 2023), ~ .x |>
  dplyr::filter(finished == 1) |>
  dplyr::select(dplyr::all_of(v_names)) |>
  dplyr::mutate(survey_yr = .y)) |>
  # Select most recent AQ for each pid
  dplyr::group_by(pid) |>
  dplyr::slice_max(order_by = survey_yr, with_ties = FALSE) |>
  dplyr::ungroup() |> 
  dplyr::select(-finished, -survey_yr)
...
```{r left join}
df <- df_wide |> 
  dplyr::left_join(dflf, by = "pid") |> 
  dplyr::left_join(df_demo, by = "pid") 
```
...
df <- df |> 
  dplyr::mutate(
    gi_sum = rowSums(dplyr::across(genderid_1:genderid_12), na.rm = T),
    so_sum = rowSums(dplyr::across(orientation_1:orientation_11), na.rm = T), 
    race_sum = rowSums(dplyr::across(race_ethn_1:race_ethn_8), na.rm = T),
    ...
    # Binary ethnoracial group: 0=white, 1=poc
    race_bin = dplyr::case_when(race_ethn_7 == 1 & race_mul == 0 ~ 1, T ~0),
    # Binary gender group: 0:cisgender, 1=transgender & nonbinary
    gi_bin = case_when(
      # Male assigned at birth AND only selected "Cisgender man" (2) or "Man" (5) or both
      saab == 1 & 
        (genderid_2 == 1 | genderid_5 == 1) &  # Selected cis man OR man
        genderid_1 == 0 & genderid_3 == 0 & genderid_4 == 0 & 
        genderid_6 == 0 & genderid_7 == 0 & genderid_8 == 0 & 
        genderid_9 == 0 & genderid_10 == 0 & genderid_11 == 0 & 
        genderid_12 == 0 ~ 0,
      # Female assigned at birth AND only selected "Cisgender woman" (3) or "Woman" (11) or both
      saab == 2 & 
        (genderid_3 == 1 | genderid_11 == 1) &  # Selected cis woman OR woman
        genderid_1 == 0 & genderid_2 == 0 & genderid_4 == 0 & 
        genderid_5 == 0 & genderid_6 == 0 & genderid_7 == 0 & 
        genderid_8 == 0 & genderid_9 == 0 & genderid_10 == 0 & 
        genderid_12 == 0 ~ 0,
      TRUE ~ 1
    )
  ) |> 
...
write.csv(
  df,
  here("..", "output", "temp_data", "df_anal_2026-03-16.csv"),
  row.names = F
)
```