...
Create a data frame that excludes students and other people who are out of the workforce for descriptive analysis and
unadjusted modeling.

```{r analytic df}
df_complete <- df |> dplyr::filter(exclude_flag == 0)

# Check that participants completed both AQ 2021 and 2022
dplyr::count(df_complete, finished_2021, finished_2022)
```

Create table that shows distribution of current ethnoracial identity, gender identity, and sexual orientation based on 
...
# Create table 
tab1 <- df_complete |>
  tidyr::pivot_longer(dplyr::all_of(v_names)) |>
  dplyr::group_by(name) |>
  dplyr::count(value) |>
  dplyr::mutate(
...
Create table that shows distribution of covariates for participants overall and by employment status in AQ 2021 and 
2022. Immigration status (`immstatus`) was only asked in LHES (at a single timepoint).

```{r table 2}
# Select relevant variables
# AQ2021
v_names21 <- c(
  "age_2021", "race_bin", "gi_bin", "educ_2021", "immstatus", "region_2021", 
...
# AQ2022
v_names22 <- c(
  "age_2022", "race_bin", "gi_bin", "educ_2022", "immstatus", "region_2022", 
  "urban_2022", "yrjobdisc_2022", "cars_2022", "phq_2022"
)

v_fct_names21 <- c("race_bin", "gi_bin", "educ_2021", "immstatus", "region_2021", "urban_2021", "yrjobdisc_2021")
v_fct_names22 <- c("race_bin", "gi_bin", "educ_2022", "immstatus", "region_2022", "urban_2022", "yrjobdisc_2022")
...
```{r phq missing}
dplyr::count(df_complete, is.na(phq_2021))
dplyr::count(df_complete, is.na(phq_2021), employed_2021)
dplyr::count(df_complete, is.na(phq_2022))
dplyr::count(df_complete, is.na(phq_2022), employed_2022)
```
...
df_complete |>
  dplyr::group_by(employed_2021, employed_2022) |>
  dplyr::summarise(
    n = n(),
...
Fit unadjusted linear regression models that examine the associations between changes in employment from 2021 to 2022 
and depressive symptoms in 2023.

```{r unadjusted regression}
# Make sure that exposure is a factor in data frame
df_complete <- df_complete |> 
  dplyr::mutate(emp_chg = as.factor(emp_chg))

m0 <- geepack::geeglm(
  phq_2023 ~ factor(unemployed_2021)*factor(unemployed_2022),
  data = df_complete,
  id = pid,
  corstr = "independence"
)
...
marginaleffects::avg_predictions(
  m0, 
  variables = c( "unemployed_2021", "unemployed_2022"), 
  hypothesis = difference ~ reference
) |> 
  arrange(estimate)
```

Fit marginal structural models. These are models that use IPW to adjust for time-varying confounding. Note that 
immigration status is only measured in LHES and treated as time-invariant in the model.

Construct stabilized IP weights for AQ 2021 and AQ 2022. Missing values for each variable are treated by creating a 
new missingness indicator to talke on the value of 1 if covariate is NA and 0 otherwise.

```{r treatment ipw}
sw_a <- WeightIt::weightitMSM(
  list(
...
cobalt::bal.tab(sw_a, stats = c("m", "ks"))
```

Check for selection bias due to censoring by comparing employment and depressive symptoms between those who are 
censored and those who are not - If people did not complete AQ 2023, they were considered LFTU and censored
...
# Select relevant variables
v_names21 <- c(
  "age_2021", "gi_bin", "race_bin", "educ_2021", "immstatus", "region_2021", 
  "urban_2021", "yrjobdisc_2021", "cars_2021", "phq_2021"
)
v_names22 <- c(
  "age_2022", "gi_bin", "race_bin", "educ_2022", "immstatus", "region_2022", 
  "urban_2022", "yrjobdisc_2022", "cars_2022", "phq_2022"
)

v_fct_names21 <- c("gi_bin", "race_bin", "educ_2021", "immstatus", "region_2021", "urban_2021", "yrjobdisc_2021")
v_fct_names22 <- c("gi_bin", "race_bin", "educ_2022", "immstatus", "region_2022", "urban_2022", "yrjobdisc_2022")
...
# Clean environment
rm(v_names21, v_fct_names21, v_names22, v_fct_names22, table_cen21, table_cen22)
```

There is evidence that people who were censored have higher depressive symptoms and prevalence of losing employment and
remaining unemployed. This suggests potential selection bias due to LFTU.

Construct stablized IPW censoring weights.

```{r lftu ipw}
sw_c <- WeightIt::weightitMSM(
  list(cens ~ age_2021 + age_2022 + educ_2021 + educ_2022 + region_2021 +
    region_2022 + urban_2021 + urban_2022 + yrjobdisc_2021 + yrjobdisc_2022 +
    cars_2021 + cars_2022 + immstatus + phq_2021 + race_bin + gi_bin + phq_2022 + 
    employed_2021 + employed_2022),
  data = df_complete,
  method = "glm",
  estimand = "ATE",
  stabilize = T,
...
# Check metrics for covariate balance
cobalt::bal.tab(sw_c, stats = c("m", "ks"))
```

Take product of treatment and censoring weights to construct the final weights that will be used to fit the marginal 
structural models.
...
```{r ipw distribution}
df_complete |> 
  dplyr::select(sw_a, sw_c, sw, sw_a_truncated, sw_c_truncated, sw_truncated) |> 
...
Estimate the joint effect of employment changes between 2021 and 2022 on depressive symptoms in 2023. Reference group
